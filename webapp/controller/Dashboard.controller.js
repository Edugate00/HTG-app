        ...
        return BaseController.extend("lrlpapp.controller.Dashboard", {
            onAfterRendering: async function () {
                ...
                oBilling.results.forEach((el) => {
                    ...
                    el.DueDate = this.getFormattedDate(
                        this.getDueDate(el.BillingDate, 7)
                    );
                    el.BillingDate = this.getFormattedDate(el.BillingDate);
                    el.MaterialDesc =
                        el.BillingHeadToItem.results[0].Description;

                    if (el.NetValue !== "") {
                        el.NetValue = Number(el.NetValue).toLocaleString(
                            "IDR-id"
                        );
                    }
                    ...
                    if (el.BillingType === "FV") {
                        ...
                        let from = this.getShortFormattedDate(
                            el.PriceListDesc.split(" - ")[0]
                        );
                        let to = this.getShortFormattedDate(
                            el.PriceListDesc.split(" - ")[1]
                        );
                ...
                sessionStorage.setItem(
                    "BILLING_FV",
                    JSON.stringify(BILLING_FV)
                );
                sessionStorage.setItem(
                    "BILLING_ZUTL",
                    JSON.stringify(BILLING_ZUTL)
                );
                ...
                MEASPOINT.forEach((el) => {
                    ...
                    if (el.MeasPointToMeasDoc.results.length !== 0) {
                        let getDateMeasDoc =
                            el.MeasPointToMeasDoc.results[0].Date;
                        ...
                        let getTimeStampDateMeasdoc = new Date(
                            getMeasdocDateFormated
                        ).getTime();

                        let currDate = new Date();
                        let last27days = new Date(
                            currDate.getTime() - 27 * 24 * 60 * 60 * 1000
                        );
                ...
                needToScanDesc = needToScanDesc.slice(
                    0,
                    needToScanDesc.length - 2
                );
                ...
                sessionStorage.setItem(
                    "NOTIF_TAGIHAN_SEWA",
                    JSON.stringify(
                        isDueDate.sort((a, b) => a.Timestamp - b.Timestamp)
                    )
                );
                sessionStorage.setItem(
                    "NOTIF_TAGIHAN_TERLAMBAT",
                    JSON.stringify(
                        hasPassed.sort((a, b) => a.Timestamp - b.Timestamp)
                    )
                );
                ...
                if (currentDate <= 10 || currentDate >= 25) {
                    ...
                    notif.push({
                        tagihanSewa: String(
                            isDueDate.length + hasPassed.length
                        ),
                    });
                ...
                const penggunaanAirModel = new JSONModel({
                    data: penggunaanAir.sort(
                        (a, b) => a.timeStamp - b.timeStamp
                    ),
                });
                const penggunaanListrikModel = new JSONModel({
                    data: penggunaanListrik.sort(
                        (a, b) => a.timeStamp - b.timeStamp
                    ),
                });
                ...
                const tilePindaiMeteran =
                    this.getView().byId("tilePindaiMeteran");
                ...
                tileBelumCetak.attachBrowserEvent(
                    "click",
                    this._onBelumCetak,
                    this
                );
                ...
                vizListrik.setModel(
                    penggunaanListrikModel,
                    "penggunaanListrik"
                );

                oComboBoxTenant.destroyItems();
                oComboBoxTenant.addItem(
                    new Item({ key: "*", text: "Semua Tenant" })
                );
            ...
            _onBelumCetak: function (oEvent) {
                const oView = this.getView();
                const belumCetak = JSON.parse(
                    sessionStorage.getItem("TO_PRINT")
                );
            ...
            onBelumCetakDialogCetak: function () {
                ...
                listTagihanSewa.forEach((el) => {
                    selectedList.push(
                        el.getBindingContext("belumCetak").getObject()
                    );
                });
            ...
            onDetailTagihanCetak: function () {
                const detailTagihanTercetak = this.getView()
                    .byId("detailListTagihan")
                    .getModel("tagihan");
                        ...
                        endButton: new Button({
                            ...
                            press: function () {
                                ...
                                if (this.byId("belumCetakDialog")) {
                                    let items = [];
                                    const oList =
                                        this.getView().byId("listBelumCetak");
                                    const oSelectedItems =
                                        oList.getSelectedItems();
                                    oSelectedItems.forEach((el) => {
                                        items.push(
                                            el
                                                .getBindingContext("belumCetak")
                                                .getObject()
                                        );
                                    });
