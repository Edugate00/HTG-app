        ...
        return BaseController.extend("lrlpapp.controller.Rental", {
            onAfterRendering: async function () {
                ...
                oRentalMaster.results.forEach((el) => {
                    ...
                    if (rentalDetail) {
                        const biayaMaintenance = Number(
                            rentalDetail.BiayaPemeliharaanPerBln
                        );
                        const biayaSewa = Number(rentalDetail.NilaiSewaPerBln);

                        rentalDetail.BiayaPemeliharaanPerBln = `${biayaMaintenance.toLocaleString(
                            "id-ID",
                            { style: "currency", currency: "IDR" }
                        )}`;
                        rentalDetail.NilaiSewaPerBln = `${biayaSewa.toLocaleString(
                            "id-ID",
                            { style: "currency", currency: "IDR" }
                        )}`;

                        el.KontrakEnd = this.getShortFormattedDate(
                            el.KontrakEnd
                        );
                        el.KontrakStart = this.getShortFormattedDate(
                            el.KontrakStart
                        );
                ...
                this.getView().setModel(
                    new JSONModel({ rentalMaster: rentalMaster }),
                    "rentalMaster"
                );
            ...
            onTagihanRentalSelect: async function (oEvent) {
                ...
                const BILLING_FV = JSON.parse(
                    sessionStorage.getItem("BILLING_FV")
                );
                const BILLING_ZUTL = JSON.parse(
                    sessionStorage.getItem("BILLING_ZUTL")
                );

                const oContext = oEvent
                    .getParameter("listItem")
                    .getBindingContext("rentalMaster");
                const oSelectedData = oContext.getObject();
                const rentalDetail =
                    oSelectedData.RentalMasterToDetail.results[0];
                ...
                this.getView().setModel(
                    new JSONModel(rentalDetail),
                    "rentalDetail"
                );
                this.getView().setModel(
                    new JSONModel({ listTagihanSewa: tagihanSewa }),
                    "listTagihanSewa"
                );
                this.getView().setModel(
                    new JSONModel({
                ...
                this.getView().setModel(
                    new JSONModel({ listTagihanAir: tagihanAir }),
                    "listTagihanAir"
                );

                const sToPageId = oEvent
                    .getParameter("listItem")
                    .getCustomData()[0]
                    .getValue();
            ...
            onSelectListTagihan: function (oEvent) {
                ...
                const oSewaContext = oEvent
                    .getSource()
                    .getBindingContext("listTagihanSewa");
                const oPengelolaanContext = oEvent
                    .getSource()
                    .getBindingContext("listTagihanPengelolaan");
                const oAirContext = oEvent
                    .getSource()
                    .getBindingContext("listTagihanAir");

                if (oSewaContext) {
                    oPath = oSewaContext.getPath().slice(17);
                    oListTagihan = this.getView()
            ...
            onOrientationChange: function (mParams) {
                const sMsg =
                    "Orientation now is " +
                    (mParams.landscape ? "Landscape" : "Potrait");
            ...
            onPengelolaanAscending: function () {
                const listTagihanPengelolaan = this.byId(
                    "listTagihanPengelolaan"
                );
