                        ...
                        for (let index = 0; index < selectedPeriodCashflow.length; index++) {
                            if (selectedPeriodCashflow[index].account.includes('CF_IN')) {
                                ...
                                // Get cashin financing acts
                                    ...
                                    selectedPeriodCashflow[index].account === 'CF_IN_Transfer_in_from_Related_Companies'
                                ) {
                                    const data = {
                                        account: selectedPeriodCashflow[index].account
                                            .replace('CF_IN_', '')
                                ...
                                // Get cashout financing acts
                                    ...
                                    selectedPeriodCashflow[index].account ===
                                        'CF_OUT_Transfer_out_to_Related_Companies' ||
                    ...
                    await this.clearContainer()
                        .then(async () => {
                            ...
                            const setSelectedPeriod = async () => {
                                ...
                                const selectedMonth = new FlexBox({width: '20%', justifyContent: 'End'}).addStyleClass(
                                    'cf-header',
                                );
                                const prevMonth = new FlexBox({width: '20%', justifyContent: 'End'}).addStyleClass(
                                    'cf-header',
                                );
                                ...
                                selectedMonth
                                    .addItem(
                                        new Text({text: `${monthList[Number(selectedPeriod) - 1]} ${selectedYear}`}),
                                    )
                            ...
                            const setHeaderTitle = (title) => {
                                ...
                                const blank =
                                    title === 'Operating Activities' ? 'cf-title-section' : 'cf-title-section-content';
                            ...
                            const displayCashflow = async (cashflow, titleHeader, isBankBalance) => {
                                if (!isBankBalance) await cashflowContainer.addItem(setHeaderTitle(titleHeader));
                                cashflow.forEach((el) => {
                                    const record = new HBox(generateNewAccountId(el.account)).addStyleClass(
                                        'cf-record',
                                    );
                                    const descCol = new FlexBox({width: '40%'}).addStyleClass('cf-header');
                                    const selectedMonthCol = new FlexBox({
                                        width: '20%',
                                        justifyContent: 'End',
                                    }).addStyleClass('cf-header');
                                    const prevMonthCol = new FlexBox({
                                        width: '20%',
                                        justifyContent: 'End',
                                    }).addStyleClass('cf-header');
                                    const varianceCol = new FlexBox({
                                        width: '20%',
                                        justifyContent: 'End',
                                    }).addStyleClass('cf-header');
                                    ...
                                    selectedMonthCol.addItem(
                                        new Text({text: el.valueSelected}).addStyleClass(boldTextInit),
                                    );
                            ...
                            const displayInCashflow = async () => {
                                await displayCashflow(inOperatingActs, 'Operating Activities', false);
                                await displayCashflow(inFinancingActs, 'Financing Activities', false);
                                await displayCashflow(inInvestingActs, 'Investing Activities', false);
                                await displayCashflow(inOtherAndTotal, '', false);
                            };

                            // call function displayCashflow for cash out only
                            const displayOutCashflow = async () => {
                                await displayCashflow(outOperatingActs, 'Operating Activities', false);
                                await displayCashflow(outFinancingActs, 'Financing Activities', false);
                            ...
                            await displayCashflow(bankBalance, '', true);
                            ...
                            await cashflowContainer.addItem(setHeaderTitle('Cash Outflows')); // call function set header to create Cash Outflows title
