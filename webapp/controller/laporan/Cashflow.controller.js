            ...
            for (let index = 0; index < selectedPeriodCashflow.length; index++) {
              if (selectedPeriodCashflow[index].account.includes("CF_IN")) {
                ...
                // Get cashin investing acts
                  ...
                  totalInSelected = selectedPeriodCashflow[index].account.includes("Total") ? selectedPeriodCashflow[index].value : 0;

                  totalInPrev = selectedPeriodCashflow[index].account.includes("Total") ? prevPeriodCashflow[index].value : 0;
                ...
                // Get cashout investing acts
                  ...
                  totalOutSelected = selectedPeriodCashflow[index].account.includes("Total") ? selectedPeriodCashflow[index].value : 0;

                  totalOutPrev = selectedPeriodCashflow[index].account.includes("Total") ? prevPeriodCashflow[index].value : 0;
            ...
            .then(async () => {
              ...
              const displayCashflow = async (cashflow, titleHeader, isBankBalance) => {
                ...
                cashflow.forEach((el) => {
                  ...
                  const boldTextInit =
                    el.account.includes("Total") || el.account === "Bank Ending Balance" || el.account === "Bank Beginning Balance" ? "cf-acc-bold" : "cf-acc";
